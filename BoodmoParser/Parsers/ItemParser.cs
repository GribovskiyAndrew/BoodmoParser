            ...
            async Task Parse()
            {
                while (!_queue.IsEmpty)
                {
                    ...
                    if (ok)
                    {
                        ...
                        var aftermarketsAndItems = aftermarkets.Zip(link3["items"], (a, l) => new { Aftermarket = a, Item = l });
                        foreach (var al in aftermarketsAndItems)
                        {
                            var sparePart = await _requestManager.Get($"https://boodmo.com/api/v1/customer/api/catalog/part/{al.Item["id"]}");

                            al.Aftermarket.PartNumber = sparePart["number"].ToString();
                        }
                        ...
                        var detailsAndItems = details.Zip(link4["items"], (d, l) => new { Detail = d, Item = l });
                        foreach (var dl in detailsAndItems)
                        {
                            var sparePart = await _requestManager.Get($"https://boodmo.com/api/v1/customer/api/catalog/part/{dl.Item["id"]}");

                            dl.Detail.PartNumber = sparePart["number"].ToString();
                        }
                        ...
                        //item.SoldBy = link2["items"] == null || (link2["items"]?.ToObject<string[]>()).Length == 0 ? "-" : link2["items"].First()["seller"]["name"].ToString();
                        //item.Price = link2["items"] == null || (link2["items"]?.ToObject<string[]>()).Length == 0 ? null : Convert.ToDouble(link2["items"][0]["price"].ToString()) / 100;
